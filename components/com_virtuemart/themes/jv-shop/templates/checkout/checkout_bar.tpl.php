...
foreach ($steps_to_do as $step ) {
	...
	if ($highlighted_step > $step[0]['step_order'] ) {
		echo '<a class="jv-a-checkout" href="'. $sess->url(SECUREURL."index.php?page=checkout.index&amp;option=com_virtuemart&amp;ship_to_info_id=$ship_to_info_id&amp;shipping_rate_id=".@$shipping_rate_id."&amp;checkout_stage=".$step[0]['step_order'] ).'">';
		...
		foreach( $step as $substep ) {
			if( $substep['step_order'] > $highlighted_step ) {
				...
				echo '<a class="jv-a-checkout" href="#'.$substep['step_name'].'">'.$substep['step_msg'].'</a>';
			}
			echo '<br />';
		}
	}
